...
getCollection().then(realtyItems => {
...
function renderNotificationItems() {
  ...
  notificationMessages.forEach(notification => {
    layout += `
        <li class='notification-popup__item ${
          notification?.read ? '' : 'read'
        }' data-id='${notification?.id}'>
            <h2 class='notification-popup__title'>Увага</h2>
            <div class='notification-popup__description'>Залишилося менше 3-x днів до зняття орендної плати в будинку <strong>${
              notification?.title
            }!</strong></div>
        </li>`;
  });
